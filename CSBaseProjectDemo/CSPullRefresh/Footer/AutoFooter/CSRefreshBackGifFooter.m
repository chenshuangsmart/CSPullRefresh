...
#import "CSRefreshBackGifFooter.h"

@interface CSRefreshBackGifFooter() {
...
@implementation CSRefreshBackGifFooter
...
- (void)setState:(CSRefreshState)state {
    ...
    if (state == CSRefreshStatePulling || state == CSRefreshStateRefreshing) {  // 正在拖拽|刷新中
        ...
        self.gifImgView.hidden = NO;
    } else if (state == CSRefreshStateNoMoreData) { // 无更多数据
